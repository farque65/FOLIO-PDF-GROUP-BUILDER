...
export function useGroups() {
  ...
  const removeSlot = useCallback((groupId: string, slotId: string) => {
    setState((s) => ({
      ...s,
      groups: s.groups.map((g) =>
        g.id === groupId ? { ...g, slots: g.slots.filter((sl) => sl.slotId !== slotId) } : g
      ),
    }));
  }, []);
  ...
  const reorderSlots = useCallback((groupId: string, fromIndex: number, toIndex: number) => {
    setState((s) => ({
      ...
      groups: s.groups.map((g) => {
        if (g.id !== groupId) return g;
        return { ...g, slots: arrayMove(g.slots, fromIndex, toIndex) };
      }),
    }));
  }, []);
  ...
  const moveSlotToGroup = useCallback(
    (fromGroupId: string, slotId: string, toGroupId: string, atIndex?: number) => {
      setState((s) => {
        const sourceGroup = s.groups.find((g) => g.id === fromGroupId);
        const slot = sourceGroup?.slots.find((sl) => sl.slotId === slotId);
        if (!slot) return s;
        return {
          ...s,
          groups: s.groups.map((g) => {
  ...
  const setSlotAsTitlePage = useCallback((groupId: string, slotId: string) => {
    setState((s) => ({
      ...
      groups: s.groups.map((g) => {
        ...
        if (idx <= 0) return g;
        return { ...g, slots: arrayMove(g.slots, idx, 0) };
      }),
    }));
  }, []);

  /** Store the print settings (including the rendered imageId) on a slot */
